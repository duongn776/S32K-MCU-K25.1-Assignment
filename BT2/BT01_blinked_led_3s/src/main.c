...
/* This function enables clock for portD */
void PORTD_EnableClock(void)
{
	PCC_PORTD |= (1 << CGC);
}
/* This function configures pin for PortD */
void PORTD_Configure(uint8_t pin)
{
...
/* This function sets GPIOD as output */
void GPIOD_SetPinOutput(uint8_t pin)
{
	GPIOD_PDDR |= (1 << pin);
}
...
int main(void)
{
	...
	while (1) {
		/* Toggle all 3 LEDs */
		GPIOD_PTOR = (1 << GPIO_PIN_0) | (1 << GPIO_PIN_15) | (1 << GPIO_PIN_16);
		delay();
	}
}
